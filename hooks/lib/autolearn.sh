...
BLUERA_SECRETS_PATTERN='api[_-]?key[[:space:]]*=|_token[[:space:]]*=|^token[[:space:]]*=|password[[:space:]]*=|secret[_-]?key[[:space:]]*=|_secret[[:space:]]*=|^secret[[:space:]]*=|-----BEGIN|AWS_[A-Z_]+[[:space:]]*=|GITHUB_TOKEN[[:space:]]*=|ANTHROPIC_API_KEY[[:space:]]*=|OPENAI_API_KEY[[:space:]]*=|HF_TOKEN[[:space:]]*=|private[_-]?key[[:space:]]*=|credential[[:space:]]*=' # ok: pattern def
